...
# Troubleshooting script for common deployment errors
...
# Check Nginx configuration if running as root
...
log "Checking frontend logs for errors..."
if command_exists pm2; then
  frontend_logs=$(pm2 logs evan-james-frontend --lines 20 --nostream 2>/dev/null)
  if [ -n "$frontend_logs" ]; then
    log "Recent frontend logs:"
    echo "$frontend_logs" | tee -a $LOG_FILE
  else
    log "No recent frontend logs found or unable to retrieve logs"
  fi
fi
...
log ""
log "For more detailed troubleshooting, refer to the documentation:"
log "- docs/DEPLOYMENT_RECOVERY_GUIDE.md"
log "- docs/SIMPLIFIED_DEPLOYMENT_GUIDE.md"
log "- NGINX_CONFIGURATION_GUIDE.md"
